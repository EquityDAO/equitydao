...
contract TellerNFT is IEquiNFT, ERC721Upgradeable, AccessControlUpgradeable {
     ...
     */
    function getTier(uint256 index) external view override returns (Tier memory tier_) {
        tier_ = _tiers[index];
    }
     ...
     */
    function getTokenTier(uint256 tokenId) external view override returns (uint256 index_, Tier memory tier_) {
     ...
     */
    function getTierHashes(uint256 tierIndex) external view override returns (string[] memory hashes_) {
     ...
     */
    function getOwnedTokens(address owner) external view override returns (uint256[] memory owned_) {
     ...
     */
    function _tokenURIHash(uint256 tokenId) internal view returns (string memory) {
